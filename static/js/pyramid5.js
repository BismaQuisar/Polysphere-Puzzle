...
async function solvePyramid() {
  ...
  const PyramidArray = newPyramidArray();
...
function renderSolution(solution) {
  const scene = new THREE.Scene(); // Assuming your Three.js scene is globally available

  solution.forEach(layer => {
    layer.forEach(sphere => {
      const { x, y, z, color } = sphere;

      // Find the corresponding sphere in the scene
      const sphereObject = scene.children.find(
        obj => obj.position.x === x && obj.position.y === y && obj.position.z === z
      );

      if (sphereObject) {
        // Update the sphere's color
        sphereObject.material.color.set(color);
      }
    });
  });
}

function newPyramidArray(){
  let newArray=[];
  for(let i=0; i< 55; i++){
    newArray[i]= '';
  }
...
function showSolutions() {
  ...
  function updateDisplay(index) {
      ...
      let i =0;
        // Loop through all spheres and update their color
      sphereObjects.forEach(sphere => {
        sphere.material.color.set(currentSolution[i]);
        i++;
      });
